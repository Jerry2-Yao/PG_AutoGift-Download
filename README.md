...
> 本說明對應版本:**v0.14**——「開禮設定 → 開幾位」新增第四個模式「**只開沒光環的**」(v0.13):掃好友列表,只開頭像**沒有藍色光環**的好友禮物(有光環代表今天還能跟他提升好友等級,開了就會推進好友關係;沒光環的是今天已經互動過的人,開他的禮物只清庫存)。想清禮物庫存、又不想動到任何一段好友等級的人用。v0.14 讓「掃列表」模式開頭捲回清單頂端時,發現已經在頂端就不再多捲、省幾秒。
>
> v0.12 起有「**開禮設定**」頁(開禮前排序開關、開幾位、明信片);v0.6 起大部分手機第一次按開始會自動校準畫面位置,不用再逐畫面「學習」。
...
- **只開禮物**:自動排序好友列表(有禮物的排前面)→ 從第一位開始逐一開禮物 → 遇到沒禮物的好友就自動停下。**開法可以在「設定 → 開禮設定」換**(見下面「開禮設定」),例如只開放很久沒開的(2 天前)禮物、或只開不會推進好友等級的(沒藍色光環的)。
- **只送禮物**:同樣邏輯,自動排序 → 逐一送禮物 → 送完了自動停下。
- **全部做完**:上面兩個接起來一次跑完(先開禮,再送禮)。
- 過程中隨時可以按懸浮鈕上的紅色按鈕**立刻停止**。
- 每輪處理的好友數量、開禮方式、動畫等待速度(標準/保守/最保守)都可以在「設定」裡調整。
...
👉 **[點我下載 PG_AutoGift-v0.14.apk](https://github.com/Jerry2-Yao/PG_AutoGift-Download/releases/download/v0.14/PG_AutoGift-v0.14.apk)**
...
「設定 → 開禮設定」可以調整「只開禮物」(以及「全部做完」的開禮那一段)怎麼跑。不改的話就是預設行為,跟舊版一樣。

### 開禮前排序

- **先排序**(預設):開禮前自動把好友照「友情禮物」排一次,有禮物的排到最前面。
- **不排序**:照 Pokémon GO 目前的好友順序,從第一位開始開。平常照名字排在後面、每次都輪不到開禮的好友,關掉排序才有機會增加互動。

### 開幾位(四選一)

| 模式 | 做什麼 | 適合 |
|---|---|---|
| **開到沒禮物**(預設) | 從第一位開始開,滑到某位沒禮物可開的好友就停 | 搭配「先排序」,一次把有禮物的都開完 |
| **固定巡 N 位** | 從第一位好友往下數 N 位,中間沒禮物的**也算一位**、照樣往下滑 | 搭配「不排序」,把開禮機會分給更多好友;要巡幾位另外選 |
| **只開兩天前** | 自動排序後掃好友列表,**只開列表上寫「2 天前」的禮物** | 每日開禮次數不夠用,想先清最久沒開的 |
| **只開沒光環的** | 自動排序後掃好友列表,**只開頭像沒有藍色光環的好友禮物** | 想清禮物庫存、但不想推進任何一段好友等級(把升等機會留給想手動挑時機的人) |

> **「只開兩天前」和「只開沒光環的」比較慢、也比較「一板一眼」**——會一小段一小段地往下捲、每次停下來確認畫面不動了才繼續判讀。列表前面若有一大票不符合條件的禮物,掃過去可能要 30～60 秒才會開始開,這是正常的。這兩個模式一定會先排序(不受上面「開禮前排序」開關影響),排完會先把清單捲回頂端再開始掃。
>
> **「只開沒光環的」有時候還是會開到有光環的**——藍色光環是靠頭像外圈的顏色判斷,遇到某些頭像 / 光線 / 動畫時機可能認錯。實測大致堪用但不是 100%,很在意「絕對不能推進某段好友關係」的話,還是手動開比較保險。

> **藍色光環是什麼**:好友頭像外圈一圈亮藍色的光,代表「今天還能跟這位好友提升好友關係(送禮 / 開禮會推進等級)」。已經互動過的好友沒有光環,開他的禮物只是把禮物收下來、不會動到好友等級。

### 明信片(圖釘)

開禮時要不要順手按圖釘、按了要不要保留明信片——選項與說明都在頁面上,跟舊版位置一樣,只是從舊的「設定」頁搬到了這裡。

---

## 如果在你的手機上跑不順

第一次按開始時 App 會自己校準畫面位置(見上面「開始使用」的說明),**大部分手機到這裡就正常了,下面整段都不用看**。
...
> 這一步大多用不到——自動校準會處理掉。真的要用的話,把五個畫面都各學一次比較保險。
...
- 沒有自動更新機制,新版本一樣是回這個頁面重新下載安裝。首頁標題下面會顯示目前版本(例如 `v0.14`),回報問題時附上這個。
