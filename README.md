...
> 本說明對應版本:**v0.12**——新增「**開禮設定**」頁:開禮的方式除了原本的「開到沒禮物為止」,還可以選「**固定巡 N 位**」或「**只開兩天前的禮物**」(給每日開禮次數不夠、想先清最久沒開的人);開禮前的自動排序也可以關掉。
>
> v0.6 起大部分手機第一次按開始會自動校準畫面位置,不用再逐畫面「學習」。
...
- **只開禮物**:自動排序好友列表(有禮物的排前面)→ 從第一位開始逐一開禮物 → 遇到沒禮物的好友就自動停下。**開法可以在「設定 → 開禮設定」換**(見下面「開禮設定」),例如只開放很久沒開的(2天前)禮物。
...
👉 **[點我下載 PG_AutoGift-v0.12.apk](https://github.com/Jerry2-Yao/PG_AutoGift-Download/releases/download/v0.12/PG_AutoGift-v0.12.apk)**
...
> **「只開兩天前」比較慢、也比較「一板一眼」**——它會一小段一小段地往下捲、每次停下來確認畫面不動了才繼續判讀。列表前面若有一大票「今天」的禮物,掃過去可能要 30～60 秒才會開始開,這是正常的。它一定會先排序(不受上面「開禮前排序」開關影響)。
...
- 沒有自動更新機制,新版本一樣是回這個頁面重新下載安裝。首頁標題下面會顯示目前版本(例如 `v0.12`),回報問題時附上這個。
