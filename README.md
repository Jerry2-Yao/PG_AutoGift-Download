...
> 本說明對應版本:**v0.13**——「開禮設定 → 開幾位」新增第四個模式「**只開沒光環的**」:掃好友列表,只開頭像**沒有藍色光環**的好友禮物(有光環代表今天還能跟他提升好友等級,開了就會推進好友關係;沒光環的是今天已經互動過的人,開他的禮物只清庫存)。想清禮物庫存、又不想動到任何一段好友等級的人用。
>
...
👉 **[點我下載 PG_AutoGift-v0.13.apk](https://github.com/Jerry2-Yao/PG_AutoGift-Download/releases/download/v0.13/PG_AutoGift-v0.13.apk)**
...
> **「只開兩天前」和「只開沒光環的」比較慢、也比較「一板一眼」**——會一小段一小段地往下捲、每次停下來確認畫面不動了才繼續判讀。列表前面若有一大票不符合條件的禮物,掃過去可能要 30～60 秒才會開始開,這是正常的。這兩個模式一定會先排序(不受上面「開禮前排序」開關影響)。
>
...
- 沒有自動更新機制,新版本一樣是回這個頁面重新下載安裝。首頁標題下面會顯示目前版本(例如 `v0.13`),回報問題時附上這個。
